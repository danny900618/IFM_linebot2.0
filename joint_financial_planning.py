...
class Joint_financial():
    ...
    def record_answer(user_id: Text, mode: Text, question: Text = None, answer: Text = None) -> bool:
        ...
        # 設定欄位值
            ...
            if user_data['question_number'] == "age":
                ...
                input_data = float(re.compile(
                    r'\d+\.?\d*').search(answer).group())
            elif user_data['question_number'] == "PMT":
                input_data = float(re.compile(
                    r'\d+\.?\d*').search(answer).group()) * 10000
            else:
    ...
    @staticmethod
    def send_result(user_id: Text, mail_instance: Mail, send_mail: bool, select_type_num: int = None) -> List[TextSendMessage]:
        """Send the joint financial data to user's email.
        ...
        # 一個結果
            ...
            message_list.append(TextSendMessage(text=return_text))
            return
        # 多個結果
        else:
            # 第二次篩選 - 是否結婚以及兒女數量
